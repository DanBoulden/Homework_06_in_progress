...
-- my checksums to be sure this worked correctley (note that the customer table has no address_id 1 or 3)
...
SELECT staff.store_id, concat("$", format(SUM(amount), 2))  AS "Total Dollars"
FROM payment
LEFT JOIN staff 
...
SELECT Category_Title AS "Category Title", SUM(Gross_Revenue) AS "Gross Revenue"
FROM rental_totals
